...
public class FXRobotHandler implements IRobot {
  ...
  private void send(Scene scene, char ch, boolean ctrl) {
    ...
    if (ctrl) {
      ...
      if (fxKeyCode != null) {
        focusNode.fireEvent(
            createKeyEvent(
                focusNode, KeyEvent.KEY_PRESSED, Character.toString(ch), fxKeyCode, true));
        focusNode.fireEvent(
            createKeyEvent(
                focusNode, KeyEvent.KEY_RELEASED, Character.toString(ch), fxKeyCode, true));
        return;
      }
    }
    ...
    focusNode.fireEvent(
        createKeyEvent(
            focusNode, KeyEvent.KEY_PRESSED, Character.toString(ch), KeyCode.UNDEFINED, ctrl));
    focusNode.fireEvent(
        createKeyEvent(
            focusNode, KeyEvent.KEY_TYPED, Character.toString(ch), KeyCode.UNDEFINED, ctrl));
    focusNode.fireEvent(
        createKeyEvent(
            focusNode, KeyEvent.KEY_RELEASED, Character.toString(ch), KeyCode.UNDEFINED, ctrl));
  }
  ...
  private KeyEvent createKeyEvent(
    ...
    return new KeyEvent(
        ...
        code,
        false,
        ctrl && controlKeyCode == CONTROL,
        false,
        ctrl && controlKeyCode == META);
  }
}
